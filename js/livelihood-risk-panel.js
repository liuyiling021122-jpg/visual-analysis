...
function renderRouteRiskChain(rows, state) {
  ...
  const risks = routeRows.map((row) => Number(row.climateRiskIndex || 0));
  ...
  container.innerHTML = `
    ...
    <div class="route-risk-chain${focusedRow ? ' has-selected-node' : ''}" role="list" aria-label="${escapeAttribute(`${primaryRouteName}${state.selectedEra}迁徙风险链`)}">
      ${renderRouteRiskChainArrows(routeRows.length)}
      ${routeRows
        .map((row, index) => renderRouteRiskChainNode(row, index, row === focusedRow))
        .join('')}
      ${focusedRow ? renderRouteRiskChainFocus(focusedRow, routeRows) : ''}
    </div>
    ${
      focusedRow
        ? ''
        : `<div class="route-risk-chain-insights">
            <span>${escapeHtml(routeRows[peakIndex].to_location)}段气候生产风险最高 ${formatNumber(risks[peakIndex], 2)}，平均采蜜率 ${formatPercent(Number(routeRows[peakIndex].goodHoneyRate || 0))}</span>
            <span>全程共有 ${highRiskCount} 个路段的气候生产风险指数达到或超过 0.80</span>
            <span>${escapeHtml(routeRows[lowIndex].to_location)}段生产风险最低 ${formatNumber(risks[lowIndex], 2)}，形成阶段性缓冲</span>
          </div>`
    }
  `;
    ...
    <section class="route-risk-chain-focus-card" aria-label="${escapeAttribute(`${routeLabel}风险解释`)}">
      ...
      <div class="route-risk-focus-content">
        <div class="route-risk-focus-metrics">
          ...
          <span>主要风险来源<strong>${getProductionRiskDriver(row)}</strong></span>
        </div>
        <div class="route-risk-nectar-placeholder" role="img" aria-label="${escapeAttribute(`${row.nectar_sources || '蜜源'}图片占位`)}">
          <span>蜜源图片</span>
          <small>${escapeHtml(row.nectar_sources || '待补充')}</small>
        </div>
      </div>
...
function renderRouteRiskFocus(state, data) {
  ...
  const routeRows = data.routeSegmentSummary.filter((row) => row.route_name === activeRouteName && eraMatches(row.era, state.selectedEra));
  const routeSummary = summarizeSelectedRoutes(routeRows);
  if (!routeRows.length) {
    container.innerHTML = `
      <header class="data-card-header">
